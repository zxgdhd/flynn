...
(
  ...
  tar c -C "${src}/test/apps/http" . | docker run -i -a stdin -a stdout -a stderr -e CONTROLLER_KEY="[[ .ControllerKey ]]" -e SLUG_IMAGE_ID="[[ .SlugImageID ]]" flynn/slugbuilder
...
func (s *ReleaseSuite) TestReleaseImages(t *c.C) {
	...
	releaseScript.Execute(&script, struct{ ControllerKey, SlugImageID string }{s.clusterConf(t).Key, slugImageID})
	...
	for _, app := range updater.SystemApps {
		...
		debugf(t, "checking new %s release is using image %s", app.Name, images[app.Name])
